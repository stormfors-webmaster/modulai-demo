...
function displayItems(data) {
  ...
  console.log(`Total Items: ${data.pagination?.total || data.items?.length || 0}`);
  ...
  data.items.forEach((item, index) => {
    console.log(`\n${"=".repeat(80)}`);
    console.log(`Item ${index + 1}: ${item.fieldData?.name || item.name || item.id}`);
    console.log("=".repeat(80));
    ...
    console.log(`Is Draft: ${item.isDraft !== undefined ? item.isDraft : 'N/A'}`);
    console.log(`Is Archived: ${item.isArchived !== undefined ? item.isArchived : 'N/A'}`);
