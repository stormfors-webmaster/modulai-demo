...
✅ Nice error output (with file + line numbers)
✅ Non-blocking warnings (image path missing, recommended fields, etc.)

You can extend it easily if you need stricter rules.

---

# ✅ `tools/validate-frontmatter.js`

> **Install dependency (already used by sync script):**
> `npm i gray-matter`
...
function validateFile(filePath) {
  ...
  if (data.link && typeof data.link === "string" && !/^https?:\/\//i.test(data.link)) {
  ...
  if (data.image && typeof data.image === "string") {
    ...
    if (!/^https?:\/\//i.test(img) && !fs.existsSync(path.join(process.cwd(), img))) {
      warn(filePath, `image path not found on disk (relative imports must resolve)`);
    }
  }
  ...
  if (data.last_update && (!isIsoDate(data.last_update) || typeof data.last_update !== "string")) {
...
# ✅ Recommended GitHub Action to run this
...
If you want stricter rules (enforce image extension, enforce slug safety, enforce that authors belong to an Authors collection, etc.), tell me and I’ll drop an expanded validator.
