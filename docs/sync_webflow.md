Here’s a drop-in **starter `tools/sync-webflow.js`** you can commit and run from your GitHub Action (Node 20). It:

* Detects changed Markdown files (or `--all`),
* Parses frontmatter + body,
* Converts **Markdown → HTML** (GFM),
* Resolves **relative image paths** to raw GitHub URLs for this commit,
* **Upserts** items via **Webflow API v2**,
* Optionally **dispatches** a `webflow_item_created` event so a separate workflow can write back the `post_id`,
* Supports `--dry-run`.

> **Install deps in `tools/`**
> `npm init -y && npm i gray-matter unified remark-parse remark-gfm remark-rehype rehype-stringify rehype-sanitize`
> (No fetch lib needed; Node 20 has global `fetch`.)

Create at: `tools/sync-webflow.js`
...
### Suggested `package.json` (inside `tools/`)

```json
{
  "name": "webflow-sync-tools",
  "private": true,
  "type": "module",
  "dependencies": {
    "gray-matter": "^4.0.3",
  ...
  "scripts": {
    "sync": "node sync-webflow.js",
    "sync:all": "node sync-webflow.js --all",
    "sync:dry": "node sync-webflow.js --dry-run --all"
  }
}
```
...
### How it maps fields (defaults)

* `title` → `name` (also builds `slug` if missing)
* Markdown body → `body_rich` (HTML)
* `image` (relative or URL) → `main_image` (URL)
* `date` → `publish_date` (ISO)
* `author` → `author_text`
* `link` → `external_link`
* `published` → `is_published` + `isDraft` inverse
* `post_id` (if present) → Update instead of Create
* Writes `repository_dispatch` on create so your **Writeback workflow** can store `post_id`.

If your Webflow field API IDs differ, just adjust `FIELD_IDS` at the top.
